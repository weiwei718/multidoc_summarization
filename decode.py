...
class BeamSearchDecoder(object):
    ...
    def __init__(self, model, batcher, vocab):
        ...
        if FLAGS.single_pass:
            ...
            if not os.path.exists(self._rouge_ref_dir): os.mkdir(
                self._rouge_ref_dir)
            self._rouge_dec_dir = os.path.join(self._decode_dir, "decoded")
            if not os.path.exists(self._rouge_dec_dir): os.mkdir(
                self._rouge_dec_dir)
    ...
    def decode(self):
        ...
        while True:
            ...
            if FLAGS.single_pass:
                self.write_for_rouge(all_original_abstract_sents, decoded_words,
    ...
    def write_for_rouge(self, all_reference_sents, decoded_words, ex_index):
        ...
        while len(decoded_words) > 0:
            ...
            sent = decoded_words[
                   :fst_period_idx + 1]  # sentence up to and including the period
            decoded_words = decoded_words[
        ...
        all_reference_sents = [[make_html_safe(w) for w in abstract] for
                               abstract in all_reference_sents]
    ...
    def write_for_attnvis(self, article, abstract, decoded_words, attn_dists,
        ...
        if ex_index is None:
            ...
            output_fname = os.path.join(self._decode_dir, 'attn_vis',
                                        'attn_vis_data%06d.json' % ex_index)
        with open(output_fname, 'w') as output_file:
            json.dump(to_write, output_file)
        logging.info('Wrote visualization data to %s', output_fname)

    def calc_importance_features(self, data_path, hps, model_save_path,
                                 docs_desired):
        ...
        while True:
            ...
            if batch is None:  # finished decoding dataset in single_pass mode
                raise Exception(
                    'We havent reached the num docs desired (%d), instead we reached (%d)' % (
                    num_documents_desired, doc_counter))
            ...
            for batch_idx, enc_states in enumerate(batch_enc_states):
                ...
                all_original_abstracts_sents = \
                batch.all_original_abstracts_sents[batch_idx]

                tokenizer = Tokenizer('english')
                # List of lists of words
                enc_sentences, enc_tokens = batch.tokenized_sents[batch_idx], \
                                            batch.word_ids_sents[batch_idx]
                enc_sent_indices = importance_features.get_sent_indices(
                    enc_sentences, batch.doc_indices[batch_idx])
                ...
                sent_representations_separate = importance_features.get_separate_enc_states(
                    self._model, self._sess, enc_sentences, self._vocab, hps)

                sent_indices = enc_sent_indices
                sent_reps = importance_features.get_importance_features_for_article(
                    enc_states, enc_sentences, sent_indices, tokenizer,
                    sent_representations_separate)
                y, y_hat = importance_features.get_ROUGE_Ls(art_oovs,
                                                            all_original_abstracts_sents,
                                                            self._vocab,
                                                            enc_tokens)
                binary_y = importance_features.get_best_ROUGE_L_for_each_abs_sent(
                    art_oovs, all_original_abstracts_sents, self._vocab,
                    enc_tokens)
...
def rouge_eval(ref_dir, dec_dir):
    ...
    rouge_args = ['-e', r._data_dir,
                  ...
                  '95',
                  '-2', '4',
                  # This is the only one we changed (changed the max skip from -1 to 4)
                  '-U',
                  '-r', '1000',
                  '-n', '4',
                  '-w', '1.2',
